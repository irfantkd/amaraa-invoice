...
const buildPrintHTML = ({
  ...
  const TH =
    "font-size:8px;letter-spacing:1.5px;text-transform:uppercase;color:#2B3A7A;padding:8px 8px;font-weight:700;text-align:left;background-color:#EEF1FA;border-bottom:2px solid #C5CDE8;border-right:1px solid #D8DEED;";
  const tableHead = () => `
    <thead>
      <tr>
        <th style="${TH}width:6%;">Sl.</th>
        <th style="${TH}width:30%;">Item Name / Code</th>
        <th style="${TH}width:8%;">Qty</th>
        <th style="${TH}width:12%;">GWT</th>
        <th style="${TH}width:12%;">Cts/Size</th>
        <th style="${TH}width:14%;">Unit Price</th>
        <th style="${TH}width:18%;border-right:none;">Amount (AED)</th>
      </tr>
    </thead>`;
  ...
  const renderRows = (chunk, startIdx) =>
    chunk
      .map((it, i) => {
        ...
        const td = `padding:7px 8px;font-size:11px;vertical-align:middle;border-bottom:1px solid #E8ECF5;border-right:1px solid #EEF1FA;background-color:${bg};`;
        return `
        <tr>
          <td style="${td}color:#aaa;">${startIdx + i + 1}</td>
          <td style="${td}font-weight:600;color:#0D1B4B;">${it.itemCode || "—"}</td>
          <td style="${td}color:#555;">${it.qty}</td>
          <td style="${td}color:#555;">${it.GWT || "—"}</td>
          <td style="${td}color:#555;">${it.cts || "—"}</td>
          <td style="${td}color:#555;">${it.price ? "AED " + Number(it.price).toFixed(2) : "—"}</td>
          <td style="${td}font-weight:600;color:#0D1B4B;border-right:none;">${it.price ? "AED " + (it.qty * Number(it.price)).toFixed(2) : "—"}</td>
        </tr>`;
      })
      .join("");
...
<div class="page" style="width:794px;background:#fff;font-family:Arial,sans-serif;font-size:12px;color:#333;overflow:hidden;">
  ...
  <div style="padding:18px 28px 20px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:4px;table-layout:fixed;border:1px solid #C5CDE8;border-radius:8px;overflow:hidden;">
      ${tableHead()}
      <tbody>${chunk.length > 0 ? renderRows(chunk, startIdx) : `<tr><td colspan="7" style="padding:40px;text-align:center;color:#999;">No items to display</td></tr>`}</tbody>
    </table>
    ${showContinued ? `<div style="text-align:center;font-size:10px;color:#bbb;font-style:italic;padding:10px 0 2px;">*** Continued on next page ***</div>` : ""}
